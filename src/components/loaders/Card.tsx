export default function Card({
    ...
    return (
      <div
        className={`top-0 block min-w-[200px] w-full xm:w-[50%] xm:min-w-[50%] sm:w-[33%] sm:min-w-[33%] md:w-[25%] md:min-w-[25%] lg:w-[20%] lg:min-w-[20%] p-2`}
      >
        <div className="relative group overflow-hidden cursor-pointer max-w-full p-3 bg-[#181818] hover:bg-[#282828] rounded-md">
          <div className="max-w-full p-3 rounded-md">
            <div className="relative overflow-hidden cursor-pointer">
              {songs?.url ? (
                <audio controls src={songs.url}></audio>
              ) : (
                <p className="text-gray-500">No audio available</p>
              )}
              <div className="absolute top-[-100%] xm:group-hover:top-0 transition-all left-0 w-full h-full flex justify-end items-end p-2">
            ...
            <div className="mt-2">
              ...
              <h6 className="text-sm font-medium line-clamp-1">
                {listeningCount.length} {listeningCount.length === 1 ? "listener" : "listeners"}
              </h6>
            </div>
          </div>
        </div>
